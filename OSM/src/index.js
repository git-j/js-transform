...
index.prototype.map = function(/* JSFunction */callback){
  ...
  var self = this;
  //CODE
  callback(null,self.source.osm.node.map(function(xml_node){
    
    return node.prototype.map.apply(xml_node,[{}]);
  }).reduce(function(previous_value, current_value, index, js_array){
    /* what is acc? */
    if ( previous_value[current_value.class] === undefined ){
      previous_value[current_value.class] = [node];
    } else {
      previous_value[current_value.class].push(current_value);
    }
